...
```
The value event log itself is defined inside the root schema in v0.2,
while each event is validated against value-event.schema.json.
This function validates:
- required top-level event log fields
- append_only must be true
- events must be a non-empty list
- each event must conform to schemas/value-event.schema.json
"""
...
```
These checks do not replace JSON Schema validation.
They ensure the example files refer to the same origin_id and trace_id.
"""
